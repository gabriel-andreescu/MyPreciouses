...
namespace {
    constexpr auto kVanillaMovieFile = "mypreciouses_fingerselect.swf";
    constexpr auto kSkyUIMovieFile = "mypreciouses_fingerselect_skyui.swf";
    constexpr auto kSkyUIRuntimePath = "_global.skyui";
    constexpr auto kClipName = "MyPreciousesFingerSelect_mc";
    constexpr auto kClipPath = "_root.MyPreciousesFingerSelect_mc";
    ...
    // CommonLib's string release resolves engine addresses after SKSE has initialized the module.
    struct Session { // NOLINT(bugprone-exception-escape)
    ...
    [[nodiscard]] RE::GPtr<RE::IMenu> GetHostMenu(const ItemMenuHost a_hostMenu) {
        ...
        switch (a_hostMenu) {
            case ItemMenuHost::kInventory: return userInterface->GetMenu(RE::InventoryMenu::MENU_NAME);
            case ItemMenuHost::kFavorites: return userInterface->GetMenu(RE::FavoritesMenu::MENU_NAME);
    ...
    void CenterClip(RE::GFxMovieView& a_movie) {
        ...
        const auto screenX = CenterOffset(visibleRect.left, visibleRect.right, kClipWidth);
        const auto screenY = CenterOffset(visibleRect.top, visibleRect.bottom, kClipHeight);

        static_cast<void>(SetNumber(a_movie, std::format("{}._x", kClipPath).c_str(), screenX));
        static_cast<void>(SetNumber(a_movie, std::format("{}._y", kClipPath).c_str(), screenY));
    }
    ...
    [[nodiscard]] bool ApplyPlatform(RE::GFxMovieView& a_movie, const RE::INPUT_DEVICE a_preferredDevice) {
        ...
        if (!InvokeClip(a_movie, "SetPlatform", args.data(), static_cast<std::uint32_t>(args.size()))) {
            SKSE::log::warn("UI: finger selector platform update failed | platform={}", platform);
            return false;
        }
    ...
    [[nodiscard]] bool ApplyLabels(RE::GFxMovieView& a_movie, const Data& a_data) {
        ...
        if (!InvokeClip(a_movie, "SetLabels", args.data(), static_cast<std::uint32_t>(args.size()))) {
            SKSE::log::warn("UI: finger selector label update failed");
            return false;
        }
    ...
    [[nodiscard]] bool ApplyRows(RE::GFxMovieView& a_movie, const Data& a_data) {
        ...
        for (std::size_t index = 0; index < a_data.rows.size(); ++index) {
            ...
            if (!InvokeClip(a_movie, "SetRow", args.data(), static_cast<std::uint32_t>(args.size()))) {
                SKSE::log::warn("UI: finger selector row update failed | index={}", index);
                applied = false;
            }
        }
    ...
    [[nodiscard]] bool ApplyPreviewSource(RE::GFxMovieView& a_movie, const Data& a_data) {
        ...
        if (!InvokeClip(a_movie, "SetPreviewSource", args.data(), static_cast<std::uint32_t>(args.size()))) {
            SKSE::log::warn("UI: finger selector preview source update failed");
            return false;
        }
    ...
    [[nodiscard]] bool ApplyCommit(RE::GFxMovieView& a_movie, const Data& a_data) {
        ...
        if (!InvokeClip(a_movie, "CommitData", args.data(), static_cast<std::uint32_t>(args.size()))) {
            SKSE::log::warn("UI: finger selector commit failed | selectedIndex={}", a_data.selectedIndex);
            return false;
        }
    ...
    [[nodiscard]] std::size_t ResolveResultIndex(
        ...
        if (a_target) {
            const auto row = std::ranges::find_if(a_data.rows, [&](const Row& a_row) {
                return a_row.target == *a_target;
            });
            if (row != a_data.rows.end()) {
                return static_cast<std::size_t>(std::distance(a_data.rows.begin(), row));
            }
        }
    ...
    public:
        void Accept(CallbackProcessor* a_processor) override {
            ...
            a_processor->Process("MyPreciousesFingerSelect_LoadMenu", LoadMenuCallback);
            a_processor->Process("MyPreciousesFingerSelect_SelectionChange", SelectionChangedCallback);
            a_processor->Process("MyPreciousesFingerSelect_Equip", EquipCallback);
            a_processor->Process("MyPreciousesFingerSelect_Cancel", CancelCallback);
        }
    ...
    [[nodiscard]] RE::FxDelegateHandler* GetHandler() {
        static auto const handler = RE::make_gptr<EmbeddedHandler>();
    ...
    [[nodiscard]] bool LoadEmbeddedMovie(RE::GFxMovieView& a_movie) {
        ...
        if (!Invoke(
            ...
            SKSE::log::warn("UI: finger selector open failed | reason=viewCreationFailed");
            return false;
        }
        ...
        SKSE::log::debug("UI: finger selector load requested | movie={} | skyui={}", movieFile, usesSkyUI);
        ...
        if (!Invoke(
            ...
            SKSE::log::warn("UI: finger selector open failed | reason=viewLoadFailed");
            return false;
        }
...
bool Show(Data a_data) {
    ...
    auto data = GetData();
    auto const menu = data ? GetHostMenu(data->hostMenu) : nullptr;
    auto const movie = menu ? menu->uiMovie : nullptr;
    ...
    if (data && menu && movie && menu->fxDelegate && handler) {
        ...
        SKSE::log::warn("UI: finger selector open failed | reason=menuUnavailable");
    }
