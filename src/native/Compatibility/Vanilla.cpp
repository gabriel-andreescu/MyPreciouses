...
namespace {
    ...
    [[nodiscard]] const FrostmoonEffectDefinition* LookupFrostmoonEffectDefinition(const std::int32_t a_ringID) {
        const auto effect = std::ranges::find_if(kFrostmoonEffects, [a_ringID](const auto& a_effect) {
            return a_effect.ringID == a_ringID;
        });

        return effect != kFrostmoonEffects.end() ? std::addressof(*effect) : nullptr;
    }
    ...
    [[nodiscard]] std::optional<std::vector<std::int32_t>> ReadAppliedRingIDs(
        ...
        if (remaining != 0) {
            SKSE::log::warn("Compatibility: Frostmoon load skipped | remaining={} | reason=unexpectedData", remaining);
            Serialization::DrainRecordData(a_intfc, remaining);
            return std::nullopt;
        }
    ...
    [[nodiscard]] bool IsFrostmoonRingRightWorn(
        ...
        const auto ringDefinition = std::ranges::find_if(kFrostmoonRings, [a_ringID](const auto& a_ring) {
            return a_ring.ringID == a_ringID;
        });
        if (ringDefinition == kFrostmoonRings.end()) {
            return false;
        }

        auto const* ring = LookupFrostmoonRing(a_data, *ringDefinition);
        return ring != nullptr && Inventory::GetRingInventoryState(a_actor, *ring).rightWorn;
    }
    ...
    [[nodiscard]] bool IsWerewolf(RE::Actor const& a_actor, RE::TESDataHandler& a_data) {
        const auto* werewolfRace = a_data.LookupForm<RE::TESRace>(kWerewolfBeastRaceFormID, kSkyrimPlugin);
        return werewolfRace != nullptr && a_actor.GetRace() == werewolfRace;
    }

    void AppendSuspendedFrostmoonRings(
        RE::TESDataHandler& a_data,
        RE::Actor& a_player,
        ...
        const auto pending = Equipment::RaceSwitchRestore::GetPendingRestore(Core::MakeActorKey(a_player));
        if (!pending || pending->raceFormID != a_player.GetRace()->GetFormID()) {
            return;
        }

        // Vanilla unequips rings after changing race. Their werewolf effects last until the player changes back.
        for (const auto& assignment : pending->assignments.byTarget) {
            const auto* ring = LookupFrostmoonRingDefinition(a_data, assignment.source.sourceFormID);
            auto const* source = ring ? LookupFrostmoonRing(a_data, *ring) : nullptr;
            if (source && Inventory::FindFormOnlySourceMatches(a_player, *source).count > 0) {
                AddUniqueKnownRingID(a_ringIDs, ring->ringID);
            }
        }
    }

    using DoCombatSpellApplyT = void (*)(RE::Actor*, RE::SpellItem*, RE::TESObjectREFR*);
    ...
    void DoCombatSpellApply(RE::Actor& a_actor, RE::SpellItem& a_spell) {
        ...
        REL::Relocation<DoCombatSpellApplyT> const doCombatSpellApply {RELOCATION_ID(37666, 38620)};
    ...
    void RemoveFrostmoonTransformEffect(RE::TESDataHandler& a_data, RE::Actor& a_actor, const std::int32_t a_ringID) {
        ...
        switch (definition->kind) {
            ...
            case FrostmoonEffectKind::kCombatSpell: {
                auto const* spell = a_data.LookupForm<RE::SpellItem>(definition->localFormID, kDragonbornPlugin);
                if (spell) {
...
void RefreshFrostmoonVirtualRings(
    ...
    auto ringIDs = ResolveVirtualFrostmoonRingIDs(*data, a_virtualRingSourceFormIDs);
    auto* player = RE::PlayerCharacter::GetSingleton();
    if (player) {
        AppendSuspendedFrostmoonRings(*data, *player, ringIDs);
    }
    {
        std::scoped_lock const lock(g_lock);
...
void HandleSpellCast(RE::Actor& a_actor, const RE::FormID a_spellFormID) {
    ...
    const auto isWerewolfChange = (werewolfChange != nullptr && werewolfChange->GetFormID() == a_spellFormID)
                                  || (hircineChange != nullptr && hircineChange->GetFormID() == a_spellFormID);
    if (!isWerewolfChange) {
        return;
    }
...
void Save(SKSE::SerializationInterface& a_intfc) {
    ...
    if (!a_intfc.OpenRecord(kRecordFrostmoonCompatibility, kFrostmoonCompatibilityVersion)) {
        SKSE::log::error("Compatibility: Frostmoon save failed | reason=openRecord");
        return;
    }

    const auto ringIDCount = static_cast<std::uint32_t>(appliedRingIDs.size());
    if (!Serialization::WriteField(a_intfc, ringIDCount)) {
        SKSE::log::error("Compatibility: Frostmoon save failed | reason=writeCount");
        return;
    }

    for (const auto ringID : appliedRingIDs) {
        if (!Serialization::WriteField(a_intfc, ringID)) {
            SKSE::log::error("Compatibility: Frostmoon save failed | ringID={} | reason=writeRingID", ringID);
            return;
        }
    }
}
...
bool TryLoadRecord(const Serialization::RecordInfo a_recordInfo, SKSE::SerializationInterface& a_intfc) {
    ...
    if (!appliedRingIDs) {
        SKSE::log::error("Compatibility: Frostmoon load failed | reason=readRecord");
        return true;
    }
