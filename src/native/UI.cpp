...
namespace {
    ...
    [[nodiscard]] std::optional<Core::ActorKey> ResolveItemMenuActorKey(RE::GFxMovieView const* a_view) {
    ...
    [[nodiscard]] bool ShouldUpdateItemMenuRowLabel(RE::GFxMovieView const* a_view) {
        return IsOpenInventoryMenuMovie(a_view) || ContainerMenu::IsOpenMovie(a_view);
    }

    void StampItemMenuRingRowData(
        RE::GFxMovieView* a_view, // NOLINT(misc-const-correctness)
    ...
    void RefreshCurrentItemMenuRowsForRing(const Core::ActorKey a_actor, const RE::FormID a_sourceFormID) {
        ...
        auto const* ring = Inventory::AsRing(RE::TESForm::LookupByID<RE::TESObjectARMO>(a_sourceFormID));
...
void RegisterItemMenuDataCallback() {
    if (const auto* scaleform = SKSE::GetScaleformInterface()) {
        ...
        SKSE::log::info("UI: item menu data callback registered");
    } else {
        SKSE::log::warn("UI: item menu data callback skipped | reason=noScaleform");
    }
}
...
void RefreshRingItemRows() {
    SKSE::GetTaskInterface()->AddUITask([] { RefreshCurrentItemMenuRows(); });
}
...
void RefreshItemRowsAfterEquipmentAction(
    ...
    SKSE::GetTaskInterface()->AddUITask([a_hostMenu, a_actor, a_sourceFormID, a_result] {
...
void QueueFavoritesRefreshAfterRingEquip() {
    SKSE::GetTaskInterface()->AddTask([] { FavoritesMenu::QueueRingRowRefresh(); });
}
}
