...
namespace {
    ...
    constexpr auto kScaleformShowItemsListPatched = "myPreciousesContainerShowItemsListPatched";
    ...
    [[nodiscard]] RE::ContainerMenu* GetOpenContainerMenu() {
        ...
        if (!userInterface || !userInterface->IsMenuOpen(RE::ContainerMenu::MENU_NAME)) {
            return nullptr;
        }

        auto const containerMenu = userInterface->GetMenu<RE::ContainerMenu>();
        return containerMenu.get();
    }

    [[nodiscard]] std::optional<int> ReadMovieIntVariable(RE::GFxMovieView const& a_view, const char* a_path) {
        RE::GFxValue value;
        ...
        constexpr auto kMinimum = static_cast<double>(std::numeric_limits<int>::lowest());
        constexpr auto kMaximum = static_cast<double>(std::numeric_limits<int>::max());
        if (number < kMinimum || number > kMaximum) {
            return std::nullopt;
        }
    ...
    [[nodiscard]] std::optional<std::uint32_t> ReadMovieUInt32Variable(
        RE::GFxMovieView const& a_view,
        const char* a_path
    ) {
        RE::GFxValue value;
        if (!a_view.GetVariable(std::addressof(value), a_path) || !value.IsNumber()) {
    ...
    [[nodiscard]] std::optional<bool> IsViewingContainerSide(RE::GFxMovieView const& a_view) {
    ...
    [[nodiscard]] std::optional<std::uint32_t> GetSelectedCategoryFlag(RE::GFxMovieView const& a_view) {
    ...
    [[nodiscard]] bool RestampVisibleRingRows(RE::ContainerMenu& a_containerMenu) {
        auto const* movie = a_containerMenu.uiMovie.get();
        auto const& runtimeData = REL::RelocateMember<RE::ContainerMenu::RUNTIME_DATA>(
    ...
    [[nodiscard]] bool InstallShowItemsListPatch(
        ...
        auto const handler = RE::make_gptr<ShowItemsListHandler>(originalFunction);
    ...
    void InstallShowItemsListPatches(RE::ContainerMenu const& a_containerMenu) {
        ...
        if (installed) {
            SKSE::log::info("UI: ContainerMenu ShowItemsList patch installed");
        }
    }
...
bool IsOpenMovie(const RE::GFxMovieView* a_view) {
    ...
    return a_view != nullptr && containerMenu != nullptr && containerMenu->uiMovie.get() == a_view;
}

std::optional<Core::ActorKey> ResolveVisibleActorKey(RE::GFxMovieView const* a_view) {
...
void OnShown(RE::ContainerMenu& a_containerMenu) {
    ...
    SKSE::GetTaskInterface()->AddUITask([] { static_cast<void>(TryRefreshOpenMenuRows()); });
}
