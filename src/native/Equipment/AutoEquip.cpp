...
namespace {
    ...
    [[nodiscard]] std::optional<Core::ActorKey> ResolveOpenFollowerTradeTarget() {
        ...
        auto const* actor = target ? target->As<RE::Actor>() : nullptr;
        if (!actor) {
            return std::nullopt;
        }
    ...
    [[nodiscard]] bool HasBondOfMatrimonyCandidate(const std::vector<Candidate>& a_candidates) {
        return std::ranges::any_of(a_candidates, [](const Candidate& a_candidate) {
            return Compatibility::IsBondOfMatrimony(a_candidate.ring);
        });
    }
    ...
    [[nodiscard]] bool IsBetterCandidate(
        ...
        if (a_prioritizeBondOfMatrimony) {
            const auto lhsBond = Compatibility::IsBondOfMatrimony(a_lhs.ring);
            const auto rhsBond = Compatibility::IsBondOfMatrimony(a_rhs.ring);
            if (lhsBond != rhsBond) {
                return lhsBond;
            }
        }
    ...
    [[nodiscard]] std::optional<std::size_t> SelectCandidateForTarget( // NOLINT(readability-function-size)
        const Core::ActorKey a_actor,
    ...
    void QueueContainerActorRefresh(
        ...
        auto* actor = a_container != 0 ? RE::TESForm::LookupByID<RE::Actor>(a_container) : nullptr;
        if (!actor) {
            return;
        }
    ...
    [[nodiscard]] std::optional<PlannedAssignment> FindPlannedAssignment(
        ...
        const auto assignment = std::ranges::find(a_plan, a_target, &PlannedAssignment::target);
        return assignment != a_plan.end() ? std::make_optional(*assignment) : std::nullopt;
    }
    ...
    [[nodiscard]] bool PlanSelectsBondOfMatrimonyForLeftRingFinger(
        ...
        return Compatibility::IsBondOfMatrimony(a_candidates[plannedLeftRing->candidateIndex].ring);
    }
    ...
    [[nodiscard]] NativeRingSlotPlan ResolveNativeRingSlotPlan(
        ...
        if (!assignment) {
            return NativeRingSlotPlan {
                .state = NativeRingSlotPlanState::kResolutionFailed,
    ...
    [[nodiscard]] bool ApplyPlan(
        ...
        const auto nativeSlotResult = nativeSlotPlan.state == NativeRingSlotPlanState::kResolutionFailed
                                          ? NativeRingSlotApplyResult {
                                                .changed = false,
                                                .failed = true,
                                                .fallbackReservedNativeRing = std::nullopt,
                                                .removedCannotWearExtraLists = {},
                                            }
        ...
        std::vector<PlannedAssignment> fallbackPlan;
        if (nativeSlotResult.failed) {
            fallbackPlan = nativeSlotResult.fallbackReservedNativeRing
        ...
        const auto appliedPlan = ResolveAppliedVirtualPlan(
            a_actorRef,
            a_candidates,
    ...
    [[nodiscard]] bool ShouldRefreshWhenUnchanged(const ReasonMask a_reasons) {
        constexpr auto kRefreshReasons = ToReasonMask(RefreshReason::kLoad)
                                         | ToReasonMask(RefreshReason::kSettingsChanged)
                                         | ToReasonMask(RefreshReason::kEquipChanged)
                                         | ToReasonMask(RefreshReason::kLoad3D);
        return (a_reasons & kRefreshReasons) != 0;
    }
    ...
    [[nodiscard]] VirtualSlots::RefreshOptions ToVirtualRefreshOptions(const ReasonMask a_reasons) {
        return VirtualSlots::RefreshOptions {
            ...
            .restoreMissingEffects = (a_reasons & ToReasonMask(RefreshReason::kLoad3D)) != 0,
        };
    }

    [[nodiscard]] bool RunRefresh(const Core::ActorKey a_actor, const ReasonMask a_reasons) {
        ...
        // Skyrim refuses to finish enchantments whose visuals are still initializing, even with forced dispel.
        const auto nativeRing = Inventory::FindRightWornRing(*actor);
        if (nativeRing && !VirtualSlots::EnchantmentEffects::CanDispelSourceEffects(*actor, *nativeRing->ring)) {
            return false;
        }
    ...
    [[nodiscard]] bool FinishRefreshIteration(const Core::ActorKey a_actor, const ReasonMask a_deferredReasons) {
        std::scoped_lock const lock(g_lock);
        auto& states = RefreshStates();
        auto const actorState = states.find(a_actor);
        if (actorState == states.end()) {
            return false;
        }

        auto& state = actorState->second;
        state.reasons |= a_deferredReasons;
        if (state.pending) {
            return true;
        }

        if (state.reasons != 0) {
            // Preserve pending work until the actor and its enchantments are ready.
            state.running = false;
        } else {
            states.erase(actorState);
        }
        return false;
    }
    ...
    void RunRefreshLoop(const Core::ActorKey a_actor) {
        for (;;) {
            ...
            const auto completed = RunRefresh(a_actor, reasons);
            if (!FinishRefreshIteration(a_actor, completed ? 0 : reasons)) {
                return;
            }
        }
    }
...
void HandleContainerMenuOpened() {
    ...
    if (target) {
        SKSE::log::debug("AutoEquip: follower trade target registered | actor={:08X}", target->referenceFormID);
    }
}
...
void HandleEquipEvent(RE::Actor& a_actor, const RE::FormID a_sourceFormID, const bool a_equipped) {
    auto const* ring = Inventory::AsRing(RE::TESForm::LookupByID(a_sourceFormID));
    if (!ring) {
        return;
    }

    // Skyrim also sends an equip event when it rejects an armor enchantment.
    // Retrying that event can loop indefinitely, including inside a paused inventory menu.
    if (a_equipped && !Inventory::GetRingInventoryState(a_actor, *ring).rightWorn) {
        return;
    }
...
void ResumeDeferredRefresh(const Core::ActorKey a_actor) {
    {
        std::scoped_lock const lock(g_lock);
        const auto actorState = RefreshStates().find(a_actor);
        if (actorState == RefreshStates().end() || actorState->second.queued || actorState->second.running) {
            return;
        }
        actorState->second.queued = true;
    }
    SKSE::GetTaskInterface()->AddTask([a_actor] { RunRefreshLoop(a_actor); });
}
