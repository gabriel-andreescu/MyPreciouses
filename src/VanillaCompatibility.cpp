...
namespace {
    ...
    [[nodiscard]] RE::TESObjectARMO* LookupFrostmoonRing(
        ...
        return a_data.LookupForm<RE::TESObjectARMO>(a_ring.localFormID, Forms::kDragonbornESM);
    }
    ...
    [[nodiscard]] std::optional<std::vector<std::int32_t>> ReadAppliedRingIDs(
        ...
        if (!ReadField(a_intfc, remaining, ringIDCount)) {
        ...
        if (ringIDCount > kMaxFrostmoonRingIDs) {
            logger::warn(
                "FrostmoonCompatibility: load skipped | count={} | max={} | reason=invalidCount",
                ringIDCount,
        ...
        for (std::uint32_t index = 0; index < ringIDCount; ++index) {
            std::int32_t ringID = 0;
            if (!ReadField(a_intfc, remaining, ringID)) {
                DrainRecordData(a_intfc, remaining);
                return std::nullopt;
            }
        ...
        if (remaining != 0) {
            logger::warn("FrostmoonCompatibility: load skipped | remaining={} | reason=unexpectedData", remaining);
            DrainRecordData(a_intfc, remaining);
            return std::nullopt;
        }
    ...
    [[nodiscard]] bool IsFrostmoonRingRightWorn(
        ...
        return ring && Inventory::GetSourceState(a_actor, *ring).rightWorn;
    }
    ...
    [[nodiscard]] bool IsWerewolf(RE::Actor& a_actor, RE::TESDataHandler& a_data) {
        const auto* werewolfRace = a_data.LookupForm<RE::TESRace>(kWerewolfBeastRaceFormID, Forms::kSkyrimESM);
    ...
    TransformApplyResult ApplyFrostmoonTransformEffect(
        ...
        if (!definition) {
            logger::warn(
                "FrostmoonCompatibility: transform apply skipped | ringID={} | reason=missingDefinition",
                a_ringID
            );
        ...
        switch (definition->kind) {
            case FrostmoonEffectKind::kPerk: {
                auto* perk = a_data.LookupForm<RE::BGSPerk>(definition->localFormID, Forms::kDragonbornESM);
                if (!perk) {
                    logger::warn(
                        "FrostmoonCompatibility: transform apply skipped | ringID={} | form={:08X} | reason=missingPerk",
                        a_ringID,
            ...
            case FrostmoonEffectKind::kSpell: {
                auto* spell = a_data.LookupForm<RE::SpellItem>(definition->localFormID, Forms::kDragonbornESM);
                if (!spell) {
                    logger::warn(
                        "FrostmoonCompatibility: transform apply skipped | ringID={} | form={:08X} | reason=missingSpell",
                        a_ringID,
                        definition->localFormID
                    );
                    return TransformApplyResult::kFailed;
...
void OnPlayerSpellCast(RE::Actor& a_actor, const RE::FormID a_spellFormID) {
    auto* data = RE::TESDataHandler::GetSingleton();
    if (!data) {
        return;
    }

    const auto* werewolfChange = data->LookupForm<RE::SpellItem>(kWerewolfChangeFormID, Forms::kSkyrimESM);
    const auto* hircineChange = data->LookupForm<RE::SpellItem>(kWerewolfChangeRingOfHircineFormID, Forms::kSkyrimESM);
    ...
    {
        std::scoped_lock lock(g_lock);
        g_transformState.pendingRingIDs = pendingRingIDs;
    }
}
...
void Save(SKSE::SerializationInterface& a_intfc) {
    ...
    if (!a_intfc.OpenRecord(kRecordFrostmoonCompatibility, kFrostmoonCompatibilityVersion)) {
        logger::error("FrostmoonCompatibility: save failed | reason=openRecord");
        return;
    }

    const auto ringIDCount = static_cast<std::uint32_t>(appliedRingIDs.size());
    if (!WriteField(a_intfc, ringIDCount)) {
        logger::error("FrostmoonCompatibility: save failed | reason=writeCount");
        return;
    }

    for (const auto ringID : appliedRingIDs) {
        if (!WriteField(a_intfc, ringID)) {
            logger::error("FrostmoonCompatibility: save failed | ringID={} | reason=writeRingID", ringID);
            return;
        }
    }
}
...
bool LoadRecord(const Serialization::RecordInfo a_recordInfo, SKSE::SerializationInterface& a_intfc) {
    ...
    if (a_recordInfo.version != kFrostmoonCompatibilityVersion) {
        logger::warn(
            "FrostmoonCompatibility: load skipped | version={} | length={} | reason=unsupportedVersion",
            a_recordInfo.version,
            a_recordInfo.length
        );
        DrainRecordData(a_intfc, a_recordInfo.length);
        return true;
    }

    auto appliedRingIDs = ReadAppliedRingIDs(a_intfc, a_recordInfo.length);
    if (!appliedRingIDs) {
        logger::error("FrostmoonCompatibility: load failed | reason=readRecord");
        return true;
    }
