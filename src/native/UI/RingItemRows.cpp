...
namespace {
    ...
    constexpr auto kScaleformSelectionKind = "myPreciousesSelectionKind";
    constexpr auto kScaleformCustomEnchantmentID = "myPreciousesCustomEnchantmentID";
    constexpr auto kScaleformCustomCharge = "myPreciousesCustomCharge";
    constexpr auto kScaleformCustomRemoveOnUnequip = "myPreciousesCustomRemoveOnUnequip";
    constexpr auto kScaleformCustomDisplayName = "myPreciousesCustomDisplayName";
    constexpr auto kScaleformCustomHasUniqueID = "myPreciousesCustomHasUniqueID";
    constexpr auto kScaleformCustomUniqueBaseID = "myPreciousesCustomUniqueBaseID";
    constexpr auto kScaleformCustomUniqueID = "myPreciousesCustomUniqueID";
    constexpr auto kScaleformCustomBlockReason = "myPreciousesCustomBlockReason";
    constexpr auto kScaleformVanillaRingSlotEquipped = "myPreciousesVanillaRingSlotEquipped";
    constexpr auto kScaleformRingBaseText = "myPreciousesBaseText";
    constexpr auto kScaleformRingBaseHadEquipState = "myPreciousesBaseHadEquipState";
    constexpr auto kScaleformRingBaseEquipState = "myPreciousesBaseEquipState";
    constexpr auto kScaleformRingBaseHadIsEquipped = "myPreciousesBaseHadIsEquipped";
    constexpr auto kScaleformRingBaseIsEquipped = "myPreciousesBaseIsEquipped";
    constexpr auto kRingRowLeftHandShortKey = "$MYPRECIOUSES_Inventory_LeftHandShort";
    constexpr auto kRingRowRightHandShortKey = "$MYPRECIOUSES_Inventory_RightHandShort";
    ...
    [[nodiscard]] bool IsVirtuallyEquippedOnHand(const Core::TargetMask& a_virtualTargets, const Core::Hand a_hand) {
        return std::ranges::any_of(Core::kVirtualTargets, [&](const auto a_target) {
            return a_target.hand == a_hand && a_virtualTargets.Contains(a_target);
        });
    }

    [[nodiscard]] int GetRingEquipState(
        const Core::TargetMask& a_virtualTargets,
        const bool a_vanillaRingSlotEquipped
    ) {
        const auto leftEquipped = IsVirtuallyEquippedOnHand(a_virtualTargets, Core::Hand::kLeft);
        const auto rightEquipped = a_vanillaRingSlotEquipped
                                   || IsVirtuallyEquippedOnHand(a_virtualTargets, Core::Hand::kRight);
    ...
    [[nodiscard]] std::vector<Core::Target> CollectRingRowTargets(
        ...
        auto const addOccupiedTargets = [&targetMask](const Core::TargetMask& a_occupiedTargets) {
        ...
        for (const auto target : Core::kVirtualTargets) {
            if (a_virtualTargets.Contains(target)) {
    ...
    [[nodiscard]] bool HasNonIndexTarget(const std::vector<Core::Target>& a_targets) {
        return std::ranges::any_of(a_targets, [](const auto a_target) {
    ...
    [[nodiscard]] bool UpdateRingRowLabel(
        ...
        auto const* ring = Inventory::AsRing(RE::TESForm::LookupByID(a_source.sourceFormID));
        const auto targets = ring ? CollectRingRowTargets(*ring, a_virtualTargets, a_vanillaRingSlotEquipped)
...
std::string GetRingDisplayName(const RE::TESObjectARMO& a_ring) {
    ...
    return name && *name != '\0' ? name : kEmptyRingDisplayName;
}
...
bool CanShowFingerSelectHint(const RE::GFxValue& a_entryObject) {
    ...
    auto const* ring = formID ? Inventory::AsRing(RE::TESForm::LookupByID<RE::TESObjectARMO>(*formID)) : nullptr;
    if (!ring) {
        return false;
    }
