...
void EventListener::Register() {
    ...
    if (!userInterface) {
        SKSE::log::critical("EventListener registration failed: UI is null");
        return;
    }

    eventSource->AddEventSink<RE::TESContainerChangedEvent>(listener);
    eventSource->PrependEventSink<RE::TESUniqueIDChangeEvent>(listener);
    ...
    userInterface->AddEventSink<RE::MenuOpenCloseEvent>(listener);

    auto* input = RE::BSInputDeviceManager::GetSingleton();
    if (!input) {
        SKSE::log::critical("EventListener registration failed: BSInputDeviceManager is null");
        return;
    }
...
EventListener::Control EventListener::ProcessEvent(
    ...
    if (!actor->IsPlayerRef()) {
        Equipment::AutoEquip::HandleEquipEvent(*actor, a_event->baseObject, a_event->equipped);
        return Control::kContinue;
    }
