...
namespace {
    ...
    constexpr auto kScaleformInventoryEquipHintPatched = "myPreciousesInventoryEquipHintPatched";
    constexpr auto kScaleformInventoryFingerSelectHintPatched = "myPreciousesInventoryFingerSelectHintPatched";
    ...
    constexpr auto kInventoryFingerHintKey = "$MYPRECIOUSES_Inventory_FingerHint";
    ...
    constexpr auto kPendingRestampRingRows = 1U << 0U;
    constexpr auto kPendingRefreshButtonHints = 1U << 1U;
    ...
    [[nodiscard]] RE::InventoryMenu* GetOpenInventoryMenu() {
        ...
        if (!userInterface || !userInterface->IsMenuOpen(RE::InventoryMenu::MENU_NAME)) {
            return nullptr;
        }

        auto const inventoryMenu = userInterface->GetMenu<RE::InventoryMenu>();
    ...
    void SetSkyUIFingerSelectHintButtonData(
        RE::GFxMovie& a_movie,
        RE::GFxValue const& a_inventoryMenu,
    ...
    public:
        ...
        void Call(Params& a_params) override {
            ...
            if (auto const* item = GetSelectedVanillaInventoryItem(*a_params.movie); item && item->data.objDesc) {
            ...
            AddFingerSelectHint(a_params, hintState);
        }

    private:
        [[nodiscard]] bool PrepareButtonArt(
            Params& a_params,
            const RingItemRows::RingHintState& a_hintState,
            RE::GFxValue& a_originalAltButtonArt
        ) const {
            auto restoreAltButtonArt = false;
            if (a_hintState.canUseEquip && a_params.thisPtr && Scaleform::CanReadMembers(*a_params.thisPtr)) {
                RE::GFxValue equipButtonArt;
                if (a_params.thisPtr->GetMember(kVanillaAltButtonArt, std::addressof(a_originalAltButtonArt))
                    && a_originalAltButtonArt.IsObject()
        ...
        void AddFingerSelectHint(Params& a_params, const RingItemRows::RingHintState& a_hintState) {
            if (!a_params.thisPtr) {
                return;
            }
            ...
            if (!Settings::GetSingleton()->AlwaysChooseFinger() && a_hintState.canShowFingerSelect) {
    ...
    [[nodiscard]] bool InstallScaleformFunctionPatch(
        ...
        auto const handler = RE::make_gptr<Handler>(originalFunction);
        RE::GFxValue function;
    ...
    [[nodiscard]] bool InstallButtonHintFunctionPatches(RE::InventoryMenu const& a_inventoryMenu) {
    ...
    void RefreshButtonHints(RE::InventoryMenu const& a_inventoryMenu) {
        auto* movie = a_inventoryMenu.uiMovie.get();
        if (!movie) {
            return;
        }
    ...
    void InvalidateInventoryListData(RE::InventoryMenu const& a_menu) {
    ...
    [[nodiscard]] std::optional<bool> RestampInventoryRingRows(RE::InventoryMenu& a_inventoryMenu) {
        auto const* movie = a_inventoryMenu.uiMovie.get();
        if (!movie) {
            return std::nullopt;
        }
