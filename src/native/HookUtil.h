...
void WriteThunkCall(REL::Relocation<> a_target) noexcept {
    T::func = a_target.template write_call<Size>(T::thunk);
}

template <typename T, std::size_t Size = 5>
void WriteThunkBranch(REL::Relocation<> a_target) noexcept {
    T::func = a_target.template write_branch<Size>(T::thunk);
}
...
[[nodiscard]] inline std::string_view GetFailureReason(const HookWriteResult a_result) {
    switch (a_result) {
        case HookWriteResult::kInsufficientTrampoline: return "trampolineUnavailable";
        case HookWriteResult::kBranchOutOfRange:       return "branchOutOfRange";
        case HookWriteResult::kSuccess:                return "none";
    }
...
[[nodiscard]] T ReadUnaligned(const std::uintptr_t a_address) {
    T value {};
    // Hook decoding reads an address resolved from the loaded executable.
...
[[nodiscard]] HookWriteResult HookExistingBranch(const std::uintptr_t a_src, const ExistingBranch& a_branch) {
    const auto destination = SKSE::stl::unrestricted_cast<std::uintptr_t>(T::thunk);
    const auto result = a_branch.patchSize == 5 ? WriteBranch<5>(a_src, destination)
                                                : WriteBranch<6>(a_src, destination);
...
[[nodiscard]] HookWriteResult HookFunctionPrologue(const std::uintptr_t a_src, const std::byte* a_originalBytes) {
    ...
    struct Patch : Xbyak::CodeGenerator {
        Patch(
            ...
            for (const auto originalByte : ::std::span {a_originalBytes, a_originalByteLength}) {
                db(::std::to_integer<::std::uint8_t>(originalByte));
            }
    ...
    const auto result = WriteBranch<BYTES>(a_src, SKSE::stl::unrestricted_cast<std::uintptr_t>(T::thunk));
    if (result != HookWriteResult::kSuccess) {
        return result;
    }
