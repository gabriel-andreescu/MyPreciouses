...
namespace {
    ...
    [[nodiscard]] RE::FormID GetRaceFormID(RE::Actor const& a_actor) {
        const auto* race = a_actor.GetRace();
        return race ? race->GetFormID() : RE::FormID {0};
    }

    void ReleaseUnusedBindings(const Core::ActorKey a_actor, const Core::TargetAssignments& a_snapshot) {
        const auto current = AssignmentStore::GetSnapshot(a_actor);
        for (const auto& assignment : a_snapshot.byTarget) {
            const auto effectSource = assignment.retainedEffectSourceFormID;
            if (effectSource != 0 && !std::ranges::any_of(current.byTarget, [effectSource](const auto& a_current) {
                    return a_current.retainedEffectSourceFormID == effectSource;
                })) {
                Papyrus::ScriptEventMirror::RemoveEffectSourceBindings(a_actor, effectSource);
            }
        }
    }

    void ClearActorState(const Core::ActorKey a_actor) {
        Core::TargetAssignments discarded;
        {
            std::scoped_lock const lock(g_lock);
            ActiveSwitches().erase(a_actor);
            auto const pending = PendingRestores().extract(a_actor);
            if (!pending.empty()) {
                discarded = std::move(pending.mapped().assignments);
            }
        }
        ReleaseUnusedBindings(a_actor, discarded);
    }
...
bool MarkClearedDuringRaceSwitch(RE::Actor const& a_actor) {
    ...
    auto& activeSwitches = ActiveSwitches();
    auto const activeIt = activeSwitches.find(actorKey);
    if (activeIt == activeSwitches.end()) {
        return false;
    }
...
bool HandleRaceSwitchComplete(RE::Actor& a_actor) {
    ...
    if (restoredSnapshot) {
        RestoreAvailableVirtualAssignments(a_actor, *restoredSnapshot);
        ReleaseUnusedBindings(actorKey, *restoredSnapshot);
        return true;
    }
...
void DiscardReplacedTargets(const Core::ActorKey a_actor, const Core::TargetMask& a_targets) {
    Core::TargetAssignments discarded;
    {
        std::scoped_lock const lock(g_lock);
        const auto pendingRestore = PendingRestores().find(a_actor);
        if (pendingRestore == PendingRestores().end()) {
            return;
        }
        for (const auto target : Core::kVirtualTargets) {
            auto& assignment = pendingRestore->second.assignments.byTarget[Core::ToIndex(target)];
            auto const* ring = Inventory::AsRing(RE::TESForm::LookupByID(assignment.source.sourceFormID));
            if (ring && a_targets.Intersects(SourceModelFootprints::GetProjectedRingGeometryTargets(*ring, target))) {
                discarded.byTarget[Core::ToIndex(target)] = std::exchange(assignment, {});
            }
        }
    }
    ReleaseUnusedBindings(a_actor, discarded);
...
std::optional<PendingRestore> GetPendingRestore(const Core::ActorKey a_actor) {
    std::scoped_lock const lock(g_lock);
    const auto pendingRestore = PendingRestores().find(a_actor);
    if (pendingRestore == PendingRestores().end()) {
        return std::nullopt;
    ...
    return PendingRestore {
        .actor = a_actor,
        .raceFormID = pendingRestore->second.raceFormID,
        .assignments = pendingRestore->second.assignments,
    };
}
...
void ClearNonPlayerState() {
    PendingRestoreMap discarded;
    {
        std::scoped_lock const lock(g_lock);
        std::erase_if(ActiveSwitches(), [](const auto& a_entry) { return !Core::IsPlayerActorKey(a_entry.first); });
        auto& pending = PendingRestores();
        for (auto pendingRestore = pending.begin(); pendingRestore != pending.end();) {
            if (Core::IsPlayerActorKey(pendingRestore->first)) {
                ++pendingRestore;
            } else {
                discarded.insert(pending.extract(pendingRestore++));
            }
        }
    }
    for (const auto& [actor, snapshot] : discarded) {
        ReleaseUnusedBindings(actor, snapshot.assignments);
    }
}
