...
[[nodiscard]] constexpr std::uint32_t MakeRecordType(
    ...
    return (static_cast<std::uint32_t>(static_cast<unsigned char>(a_first)) << 24U)
           | (static_cast<std::uint32_t>(static_cast<unsigned char>(a_second)) << 16U)
           | (static_cast<std::uint32_t>(static_cast<unsigned char>(a_third)) << 8U)
