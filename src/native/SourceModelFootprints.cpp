...
namespace {
    constexpr auto kLeftFingerNodePrefix = std::string_view {"NPC L Finger"};
    constexpr auto kRightFingerNodePrefix = std::string_view {"NPC R Finger"};
    constexpr auto kLeftFingerCodePrefix = std::string_view {" [LF"};
    constexpr auto kRightFingerCodePrefix = std::string_view {" [RF"};
    constexpr auto kLeftHandNode = std::string_view {"NPC L Hand [LHnd]"};
    constexpr auto kRightHandNode = std::string_view {"NPC R Hand [RHnd]"};
    constexpr auto kNodeCodeSuffix = std::string_view {"]"};
    ...
    [[nodiscard]] std::uint32_t GetSkinBoneCount(const RE::NiSkinInstance& a_skin) {
        return a_skin.skinData && a_skin.skinData->GetBoneCount() > 0 ? a_skin.skinData->GetBoneCount()
                                                                      : a_skin.numMatrices;
    }
    ...
    [[nodiscard]] const char* GetSkinBoneName(const RE::NiSkinInstance& a_skin, const std::uint32_t a_index) {
        ...
        const std::span<RE::NiAVObject* const> bones {a_skin.bones, GetSkinBoneCount(a_skin)};
        const auto* bone = bones[a_index];
    ...
    [[nodiscard]] std::uint32_t BuildPartitionVertexKey(
        ...
        if (a_partition.vertexMap) {
            const std::span<const std::uint16_t> vertexMap {a_partition.vertexMap, a_partition.vertices};
            const auto mappedVertex = vertexMap[a_vertexIndex];
        ...
        return kPartitionLocalVertexFlag | (static_cast<std::uint32_t>(a_partitionIndex) << 16U) | a_vertexIndex;
    }

    void CollectPartitionWeights(
        const RE::NiSkinInstance& a_skin,
        const RE::NiSkinPartition::Partition& a_partition,
        const std::size_t a_partitionIndex,
        const std::uint32_t a_boneCount,
        const std::uint32_t a_totalVertexCount,
        std::vector<SkinVertexWeight>& a_weights
    ) {
        const auto weightsPerVertex = static_cast<std::size_t>(a_partition.bonesPerVertex);
        const auto influenceCount = static_cast<std::size_t>(a_partition.vertices) * weightsPerVertex;
        const std::span<const float> weights {a_partition.weights, influenceCount};
        const std::span<const std::uint8_t> bonePalette {a_partition.bonePalette, influenceCount};
        const std::span<const std::uint16_t> bones {a_partition.bones, a_partition.numBones};
        for (std::uint16_t vertexIndex = 0; vertexIndex < a_partition.vertices; ++vertexIndex) {
            const auto
                vertexKey = BuildPartitionVertexKey(a_partition, a_partitionIndex, vertexIndex, a_totalVertexCount);

            for (std::uint16_t weightIndex = 0; weightIndex < a_partition.bonesPerVertex; ++weightIndex) {
                const auto weightOffset = (static_cast<std::size_t>(vertexIndex) * weightsPerVertex) + weightIndex;
                const auto weight = weights[weightOffset];
                if (weight <= kMinimumSkinWeight) {
                    continue;
                }

                const auto partitionBoneIndex = bonePalette[weightOffset];
                if (partitionBoneIndex >= a_partition.numBones) {
                    continue;
                }

                const auto skinBoneIndex = bones[partitionBoneIndex];
                if (skinBoneIndex >= a_boneCount) {
                    continue;
                }
    ...
    void CollectSkinPartitionWeights(
        ...
        for (std::size_t partitionIndex = 0; partitionIndex < partitionCount; ++partitionIndex) {
            ...
            if (HasPartitionWeights(partition)) {
                CollectPartitionWeights(
                    a_skin,
                    partition,
                    partitionIndex,
                    boneCount,
                    a_skinPartition.vertexCount,
                    a_weights
                );
            }
        }
    ...
    [[nodiscard]] bool HasRingPartition(RE::BSDismemberSkinInstance& a_skin) {
        ...
        return std::ranges::any_of(partitions, [](const auto& a_partition) {
            return a_partition.slot == kRingBodyPart;
        });
    }
    ...
    void CollectSourceTargetsFromNodeNames(RE::NiAVObject& a_root, Core::TargetMask& a_sourceTargets) {
        RE::BSVisit::TraverseScenegraphObjects(std::addressof(a_root), [&](RE::NiAVObject* a_object) {
            if (a_object && !a_object->name.empty()) {
                if (const auto parsedBone = ParseFingerBoneName(a_object->name.c_str())) {
                    a_sourceTargets.Add(ToTarget(*parsedBone));
                }
            }
    ...
    [[nodiscard]] ModelGeometryScan ScanModelGeometry(RE::NiAVObject& a_root) {
        ...
        RE::BSVisit::TraverseScenegraphGeometries(std::addressof(a_root), [&](RE::BSGeometry* a_geometry) {
            ...
            auto const* skin = geometryData.skinInstance.get();
            if (!skin) {
                return RE::BSVisit::BSVisitControl::kContinue;
            }
            foundSkin = true;
            ...
            // Finger weights identify rings that use accessory slots such as 51.
            if (skinIsRing) {
            ...
            } else if (dismemberSkin && HasRingPartition(*dismemberSkin)) {
                foundRingGeometry = true;
            }
        ...
        // Exported skeleton nodes can have no vertices attached to them.
        // A skinned model's occupied fingers come from its meaningful weights.
        if (!foundSkin) {
            CollectSourceTargetsFromNodeNames(a_root, scan.sourceTargets);
            foundRingGeometry = !scan.sourceTargets.Empty();
        }
    ...
    [[nodiscard]] ArmorGeometryFootprint DetectArmorGeometryFootprint(const RE::TESObjectARMO& a_armor) {
        ...
        for (const auto& path : GetSourceModelPaths(a_armor)) {
            ...
            // Alternate-race addons can supply body meshes alongside a ring model.
            footprint.hasRingModel = true;
...
namespace {
    [[nodiscard]] ArmorGeometryFootprint GetArmorGeometryFootprint(const RE::TESObjectARMO& a_armor) {
        ...
        {
            std::scoped_lock const lock(g_cacheLock);
...
bool HasRingModel(const RE::TESObjectARMO& a_armor) {
    return GetArmorGeometryFootprint(a_armor).hasRingModel;
}
